...
$(document).on('ready', function(){
                    ...
                    carga_Clientes().then(function(clientes){
                           ...
                           json_clientes = clientes;
                              ...
                              for (i in clientes){
                                     ...
                                     item.setAttribute('data-id',clientes[i].Id);
                                     item.setAttribute('data-telefono',clientes[i].Telefono);
                                     item.setAttribute('data-direccion',clientes[i].Direccion);
                                     item.setAttribute('data-ciudad',clientes[i].Ciudad);
                                     item.setAttribute('data-email',clientes[i].Email);
    ...
    $('div#listClientes').on('click','div.item',function(){
                   ...
                   // document.getElementById('data_email').innerHTML = email;
                    ...
                    $.post('ctrlsaldo',{A:2,Id_cliente:id_cliente},function(saldo){
                        console.log(saldo);

                        saldo = JSON.parse(saldo)
                        if (saldo.Saldo){
                          document.getElementById('valor_saldo').innerHTML = saldo.Saldo
                          document.getElementById('fecha_saldo').innerHTML = saldo.Fecha
                        }else{
                          document.getElementById('valor_saldo').innerHTML = '--'
                          document.getElementById('fecha_saldo').innerHTML = '--'
                        }
