...
function set_valor_formula(pmtPrecio_unidad,pmtCantidad,pmtDescuento,pmtIva){
                   ...
                   var precio = pmtPrecio_unidad*(1 - pmtDescuento)*pmtCantidad;
                    ...
                    /* Resumen factura  */
                    gValor_neto =  gValor_neto + parseInt(precio/(1+iva));
                    gDescuento = gDescuento + parseInt(pmtDescuento*pmtCantidad*pmtPrecio_unidad);
                    gSubtotal = gValor_neto - gDescuento;
                    gIva = gIva + (pmtPrecio_unidad*pmtCantidad - parseInt(precio/(1+iva))) ;
...
$(document).on('ready', function(){
    ...
    function getConsecutivo(){
      //  document.getElementById('txtFactura').value = consecutivo_Factura;
        ...
        $.post('ctrlfactura',{Action:0,Fecha:fecha},function(r){
            ...
            if(consecutivo<10){
                consecutivo_Factura = '000'+consecutivo;
            }else if(consecutivo>=10 && consecutivo<100){
                consecutivo_Factura = '00'+consecutivo;
            }else if(consecutivo>=100 && consecutivo<1000){
                consecutivo_Factura = '0' +consecutivo;
            }else{
    ...
    $('div.descripcion_factura').on('click','div#btnEliminar_factura',function(){
            
            var id_factura = document.getElementById('fatura_detalle').getAttribute('data-id_factura');
            
            $.post('ctrlfactura',{Action:13,Id_factura:id_factura},function(result){
            
                    console.log('Eliminando factura... ' + result);
                    
                    if(result>0){
                        document.getElementById('fd_cliente').innerHTML = 'Factura eliminada correctamente!... presiona F5';
                        getFacturas_venta();
                    }
    ...
    $('div.contentButton').on('click','div#btnAddItem',function(){
                ...
                /* Validación ---> Cajas de texto */
                   ...
                   /* Resumen factura  */
                        ...
                        gSubtotal = gValor_neto - gDescuento;
                        gIva = gIva + (precio_unidad*cantidad - parseInt(precio/(1+iva))) ;
                        gTotal = gSubtotal + gIva;

                       console.log('Iva: ' + gIva);
...
function facturas_a_vencer(){
   console.log('Facturas a vencer!')
   
    $.post('ctrlfactura',{Action: 18}, function(r){
        
        var json = jQuery.parseJSON($.trim(r));

            for(i in json){
                 var fila = document.createElement('div');
                     fila.setAttribute('class','fila_vencer');
               ...
               var fecha_pago = document.createElement('div');    
                   ...
                   fecha_pago.innerHTML = json[i].Fecha_pago;
                    ...
                    document.getElementById('datafacturas_vencer').appendChild(fila); 
            }
...
$('div#frm_vence').on('click',function(){
    var estado = parseInt($('div#datafacturas_vencer').attr('data-estado'));
    if(estado>0){
        $('div#datafacturas_vencer').addClass('hidden');
        $('div#datafacturas_vencer').attr('data-estado','0')
    }else{
        $('div#datafacturas_vencer').removeClass('hidden');
        $('div#datafacturas_vencer').attr('data-estado','1')
    }
