...
public class FacturaEncabezadoDAO {
    ...
    /* Setters */
    public void setList(LinkedList<FacturaEncabezadoVO> list) {
        this.list = list;
    }

    public void setId_encabezado_ultimo(int id_encabezado) {
        this.id_encabezado_ultimo = id_encabezado;
    }

    public void setId_detalle_ultimo(int id_detalle) {
        this.id_detalle_ultimo = id_detalle;
    }

    /* Getters */
    public LinkedList<FacturaEncabezadoVO> getList() {
        return list;
    }

    public int getId_encabezado_ultimo() {
        return this.id_encabezado_ultimo;
    }

    public int getId_detalle_ultimo() {
        return this.id_detalle_ultimo;
    }
...
//        Factura_encabezado.setVencimiento("04/01/2017");
    ...
    public String getFacturasByIdCliente(int IdCliente) {

        Conexion sql = new Conexion();
        JSONArray list_facturas = new JSONArray();/* Lista de clientes */

        try {
            PreparedStatement cstmt = sql.getConexion().prepareStatement("{call dbo.spd_F_FacturasByIdCliente(?)}",
                    ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            ...
            while (r.next()) {
                ...
                factura.put("Numero", r.getInt("Numero"));
                factura.put("Estado", r.getInt("Estado"));
                factura.put("Forma_pago", r.getInt("Forma_pago"));
                factura.put("Valor", r.getFloat("Valor"));
                factura.put("Fecha", r.getString("Fecha"));
                list_facturas.add(list_facturas.size(), factura);
            }/* end while */
 ...
 /* Close connection */
        ...
        return list_facturas.toJSONString();

    }

    public String setCerrarFacturas(String ids_factura) {

        Conexion sql = new Conexion();
        String respuesta = "";/* Lista de clientes */

        try {
            PreparedStatement cstmt = sql.getConexion().prepareStatement("{call dbo.spd_CerrarFacturas(?)}",
                    ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            cstmt.setString(1, ids_factura);
            ResultSet r = cstmt.executeQuery();
